...
const UserInfoForm = ({ onSubmit, initialValues = {}, mode = 'create' }) => {
  ...
  const calculatePVC = (data) => {
    try {
      ...
      // 2. 수식 적용: PVC = 0.35×|b/a| + 0.25×|d/a| + 0.40×|a-e|
      const term1 = 0.35 * ba;    // 0.35×|b/a|
      const term2 = 0.25 * da;    // 0.25×|d/a|
      const term3 = 0.40 * ae;    // 0.40×|a-e|

      const pvc = term1 + term2 + term3;
  ...
  const calculateBV = (data) => {
    try {
      ...
      const diff = Math.abs(ad - ac);          // |((a-d)-(a-c))|

      // 2. 새로운 수식 적용: BV = 0.6×|c/a| + 0.4×|((a-d)-(a-c))|
      const term1 = 0.6 * ca;        // 0.6×|c/a|
      const term2 = 0.4 * diff;      // 0.4×|((a-d)-(a-c))|
      
      const bv = term1 + term2;
